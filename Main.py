...
if selected == "Elevator Status":
    ...
    for elevator in lines:
        st.write(f"{elevator}: Line length is {st.session_state[f'line_{elevator}']}")
    ...
    if st.checkbox("I want to add my approximate amount of people in line for an elevator"):
        ...
        if st.button("Submit", key="submit_line"):
            ...
            st.success(f"Line length for Elevator {elevator} updated. Refresh to look at the updated line length.")
            st.rerun()
